...
(defvar chezmoi_bin "~/.local/bin/chezmoi")
...
(defun chezmoi|list-changed ()
  (with-temp-buffer
    (let ((files '())
          (search-p t))
      (shell-command (concat chezmoi_bin " diff --exclude=externals") (current-buffer))
      (save-excursion
        (goto-char (point-min))
...
(defun chezmoi|changed-p (f)
  "TODO."
  (string-equal "different\n" (shell-command-to-string (concat chezmoi_bin " verify " f " || echo \"different\""))))
...
Note: Does not run =chezmoi merge=."
  ...
  (let* ((selected-file (completing-read "Select a dotfile to merge:" (chezmoi|list-changed)))
         (source-file (shell-command-to-string-no-line (concat chezmoi_bin " source-path " selected-file))))
    (ediff-files selected-file source-file)))

(defun chezmoi|magit-status ()
  "Show the status of the chezmoi source repository."
  (interactive)
  (magit-status (shell-command-to-string-no-line (concat chezmoi_bin " source-path"))))
...
(defun chezmoi|write ()
  ...
  (interactive)
  (shell-command (concat chezmoi_bin " apply " chezmoi|selected-file)))
